...
util.program.spawn_at_startup("xiate")

-- These options are applied to all windows.
way_cooler.windows = {
  gaps = { -- Options for gaps
    size = 20, -- The width of gaps between windows in pixels
  },
  borders = { -- Options for borders
    size = 1, -- The width of the borders between windows in pixels
...
local keys = {
  -- Open dmenu
  key({ mod }, "space", util.program.spawn_once("rofi -i modi drun,run.window -show drun")),

  -- Open terminal
  key({ mod }, "return", util.program.spawn_once("xiatec")),
...
for i = 1, 9 do
  ...
  table.insert(keys,
               key({ "Control", "Alt" }, tostring(i), "switch_workspace_" .. i))
  table.insert(keys,
